...
import { useIsMobile } from "@woldui/react/hooks/use-mobile"
...
import {
  ...
  TooltipContent,
  TooltipTrigger,
} from "@woldui/react/components/ui/tooltip"
  ...
  return (
    ...
    >
      ...
      <div
        ...
        className={cn(
          "relative w-(--sidebar-width) bg-transparent transition-[width] duration-[250ms] ease-[cubic-bezier(0.36,0.66,0.4,1)] ease-linear",
      ...
      <div
        ...
        data-side={side}
        className={cn(
          "fixed inset-y-0 z-10 hidden h-svh w-(--sidebar-width) transition-[left,right,width] duration-[250ms] ease-[cubic-bezier(0.36,0.66,0.4,1)] ease-linear data-[side=left]:left-0 data-[side=left]:group-data-[collapsible=offcanvas]:left-[calc(var(--sidebar-width)*-1)] data-[side=right]:right-0 data-[side=right]:group-data-[collapsible=offcanvas]:right-[calc(var(--sidebar-width)*-1)] md:flex",
...
function SidebarGroupLabel({
  ...
  return (
    <Comp
      ...
      className={cn(
        "flex h-8 shrink-0 items-center rounded-md px-2 text-xs text-sidebar-foreground/70 ring-sidebar-ring outline-hidden transition-[margin,opacity] duration-[250ms] ease-[cubic-bezier(0.36,0.66,0.4,1)] ease-linear group-data-[collapsible=icon]:-mt-8 group-data-[collapsible=icon]:opacity-0 focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0",
        className
      )}
      {...props}
    />
...
function SidebarMenuSkeleton({
  ...
  /*
   * A width between 50% and 90%, varied per skeleton so a stack of them does not look
   * like a bar chart.
   *
   * It is derived from useId rather than Math.random. A `useState(Math.random())`
   * initialiser runs on BOTH the server and the client and returns a different number
   * each time, so the prerendered HTML and the first client render disagree — and React
   * does not repair mismatched style attributes, it just warns and keeps the server's
   * value. useId is guaranteed stable across that boundary, so hashing it keeps the
   * variety without the mismatch.
   */
